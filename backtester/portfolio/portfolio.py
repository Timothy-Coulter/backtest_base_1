...
class PoolState:
    ...
    positions: dict[str, Any] | None = None  # Add missing attribute
...
class Position:
    ...
    total_cost: float | None = None  # Will be auto-calculated in __post_init__
...
class DualPoolPortfolio:
    ...
    def process_tick(
        self, timestamp: Any, current_price: float, day_high: float, day_low: float
    ) -> dict[str, Any]:
    ...
    def _process_pool_pnl(
        self, pool: PoolState, current_price: float, day_high: float, day_low: float
    ...
    def _redistribute_gains(
        self, base_pnl: float, alpha_pnl: float, current_price: float
    ) -> tuple[float, float]:
        """Redistribute gains between pools.
    ...
    def add_base_position(self, symbol: str, quantity: float, price: float, timestamp: Any) -> bool:
        ...
        if self.base_pool.available_capital >= margin_required:
            ...
            self.logger.debug(
                f"Added {symbol} position to base pool: {quantity}@{price:.4f}, margin: {margin_required:.2f}"
            )
            return True
        return False

    def add_alpha_position(
        self, symbol: str, quantity: float, price: float, timestamp: Any
    ) -> bool:
        ...
        if self.alpha_pool.available_capital >= margin_required:
            ...
            self.logger.debug(
                f"Added {symbol} position to alpha pool: {quantity}@{price:.4f}, margin: {margin_required:.2f}"
            )
            return True
        return False
...
class GeneralPortfolio:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initialized general portfolio with ${initial_capital:.2f} capital, max {max_positions} positions"
        )
    ...
    def close_position(
        self, symbol: str, price: float, timestamp: Any, quantity: float | None = None
    ) -> bool:
    ...
    def rebalance(
        self, target_allocation: dict[str, float], tolerance: float = 0.02
    ) -> dict[str, Any]:
                ...
                if symbol in self.positions:
                    ...
                    if trade_quantity > 0:
                        ...
                        if self.can_add_position(symbol):
                            self.add_position(
                                symbol, trade_quantity, current_price, pd.Timestamp.now()
                            )
                            trades.append(
                                {'action': 'BUY', 'symbol': symbol, 'quantity': trade_quantity}
                            )
                    else:
                        # Sell
                        self.close_position(
                            symbol, current_price, pd.Timestamp.now(), abs(trade_quantity)
                        )
                        trades.append(
                            {'action': 'SELL', 'symbol': symbol, 'quantity': abs(trade_quantity)}
                        )

        return {'trades': trades, 'new_allocation': self.get_allocation()}
    ...
    def process_tick(self, timestamp: Any, market_data: dict[str, pd.DataFrame]) -> dict[str, Any]:
        """Process a market tick and update portfolio state.
                ...
                if not current_data.empty:
                    ...
                    if update_result.get('should_close', False):
                        self._close_position(
                            symbol, update_result['close_reason'], current_price, timestamp
                        )
                        continue
    ...
    def open_position(
        ...
        commission = (
            margin_required * self.commission_rate
        )  # Commission on margin, not full position
        spread_cost = 0.0
        slippage = 0.0
