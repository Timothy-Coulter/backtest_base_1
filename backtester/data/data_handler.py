...
def get_data(
    ticker: str, start_date: str = "1990-01-01", end_date: str = "2025-01-01", interval: str = "1mo"
) -> pd.DataFrame:
    """Compatibility function for existing code.
...
class DataLoader:
    ...
    def load_from_source(
        self, source: str, ticker: str, start_date: str, end_date: str, interval: str
    ) -> pd.DataFrame:
    ...
    def load_from_database(
        self, db_type: str, db_path: str, query: str, params: list[str]
    ) -> pd.DataFrame:
...
class DataValidator:
    ...
    def validate_price_movement(self, data: pd.DataFrame) -> bool:
        ...
        return bool((data[["Open", "High", "Low", "Close"]] > 0).all().all())
    ...
    def check_data_completeness(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        return {"missing_percentage": missing_percentage, "gap_analysis": gap_analysis}
...
class DataPreprocessor:
    ...
    def calculate_returns(
        self, data: pd.DataFrame, column: str = "Close", method: str = "simple"
    ) -> pd.DataFrame:
        """Calculate returns."""
        result = data.copy()
        if column in data.columns:
