...
class TestMarketDataIntegration:
    ...
    def test_missing_data_handling_imputation(self, integration_test_config):
        ...
        try:
            ...
            assert (
                imputed_na_count <= original_na_count
    ...
    def test_data_quality_consistency_checks(self, sample_market_data, integration_test_config):
        ...
        assert any(
            'missing' in issue.lower() for issue in issue_types
        ), "Should detect missing values"
        assert any(
            'invalid' in issue.lower() or 'negative' in issue.lower() for issue in issue_types
    ...
    def test_data_source_fallback_mechanisms(self, integration_test_config):
        ...
        with (
            ...
            try:
                data = data_handler.get_data_with_fallback("TEST", "2020-01-01", "2020-01-02", "1d")
                ...
                assert (
                    "fallback" in str(e).lower() or "cache" in str(e).lower()
...
class TestTechnicalIndicatorIntegration:
    ...
    def test_sma_ema_calculation_accuracy(self, sample_market_data, integration_test_config):
        ...
        pd.testing.assert_series_equal(sma_20_expected.dropna(), sma_20_actual.dropna(), atol=1e-10)
        ...
        assert ema_recent_diff <= sma_recent_diff * 1.5, "EMA should be more responsive than SMA"
    ...
    def test_rsi_calculation_consistency(self, sample_market_data, integration_test_config):
        ...
        # For RSI calculation, we need gains and losses (calculated but not used)
        # Just verify we can calculate them without using them
    ...
    def test_multi_timeframe_indicator_consistency(
        self, sample_market_data, integration_test_config
    ):
    ...
    def test_indicator_performance_impact(self, large_dataset, integration_test_config):
        ...
        assert (
            calculation_time < max_expected_time
...
class TestDataValidationPipeline:
    ...
    def test_data_completeness_validation(self, sample_market_data, integration_test_config):
        ...
        assert (
            completeness_report['completeness_ratio'] >= 0.99
        ...
        assert (
            gapped_completeness['completeness_ratio'] < 1.0
        ), "Gapped data should have <100% completeness"
        assert len(gapped_completeness['missing_periods']) > 0, "Should identify missing periods"
    ...
    def test_statistical_properties_validation(self, sample_market_data, integration_test_config):
        ...
        assert isinstance(
            outlier_detection['outlier_count'], int
        ), "Outlier count should be integer"
