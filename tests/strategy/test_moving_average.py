...
class TestSimpleMovingAverageStrategyIntegration:
    ...
    def test_simple_strategy_bullish_signal_generation(self):
        ...
        # Strong uptrend data
        data = pd.DataFrame({'Close': [100, 101, 102, 105, 108, 111, 114, 117, 120, 123]})
    ...
    def test_simple_strategy_trend_tracking(self):
        ...
        data = pd.DataFrame({'Close': [100, 99, 98, 97, 96, 97, 98, 99, 100, 101, 102, 103]})
    ...
    def test_simple_strategy_custom_parameters(self):
        ...
        strategy = SimpleMovingAverageStrategy(
            name="CustomSimple", ma_short=10, ma_long=30, leverage=2.0
        )
...
@pytest.mark.parametrize(
    "short_period,long_period,data_pattern",
    [(2, 4, "uptrend"), (3, 6, "downtrend"), (2, 3, "volatile")],
)
def test_strategies_with_different_periods(short_period, long_period, data_pattern):
    ...
    data = pd.DataFrame(
        {'Close': prices, 'High': [p * 1.05 for p in prices], 'Low': [p * 0.95 for p in prices]}
    )
