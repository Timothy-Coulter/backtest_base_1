...
try:
    ...
    from backtester.execution.order import Order, OrderSide, OrderType
...
class TestSimulatedBroker:
    ...
    def test_execute_market_order(self):
        ...
        data = pd.DataFrame(
            {'Open': [400.0], 'High': [401.0], 'Low': [399.0], 'Close': [400.0], 'Volume': [1000]}
        )
        broker.set_market_data('SPY', data)
    ...
    def test_sell_order_execution(self):
        ...
        # Create sell order
        order = Order(symbol='SPY', side=OrderSide.SELL, order_type=OrderType.MARKET, quantity=5)
